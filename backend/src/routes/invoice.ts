...
try {
const where: { academyId?: number; month?: string; status?: InvoiceStatus } = {};
...
try {
const invoices = await prisma.invoice.findMany({
where: { academyId: parseInt(academyId as string), status: 'UNPAID' as InvoiceStatus },
include: {
student: { select: { id: true, name: true, phone: true } }
},
orderBy: { dueDate: 'asc' }
...
where: { id },
data: { status: 'PAID' as InvoiceStatus, paidAt: new Date() }
});
